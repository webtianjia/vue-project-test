<template>
  ...
  <el-breadcrumb separator="/">
    ...
    <el-breadcrumb-item >云教材课件管理</el-breadcrumb-item>
  </el-breadcrumb>
    <div class="content">
        <el-form :inline="true" class="demo-form-inline">
            ...
            <el-form-item>
                <el-input size="small" placeholder="输入课件名称查找" v-model="form.title" clearable></el-input>
            </el-form-item>
            <el-form-item>
                <el-button type="primary" size="small" @click="getList">查询</el-button>
                <el-button type="primary" size="small" @click="$router.push('/home/addCourseware')">上传课件</el-button>
            </el-form-item>
        </el-form>
      <el-table
        ref="multipleTable"
        ...
        <el-table-column label="阅读量" align="center">
          <template slot-scope="scope">
             <div>{{scope.row.numof_comment}}</div>
             <div>                  
                  <el-button type="text" @click="showDialog(scope.row.id,scope.row.numof_comment)">修改阅读量
                  </el-button>
              </div>
          </template>
        </el-table-column>
        <el-table-column label="操作" align="center">
          <template slot-scope="scope">
            <div>
                <div>
                  <el-button type="text"
                             @click="$router.push({name:'addCourseware',params:{id:scope.row.id,classification:scope.row.type}})">
                    编辑
                  </el-button>
                ...
                <div>
                  <el-button type="text" @click="$router.push({name:'kejianCommentList',params:{id:scope.row.id,title:scope.row.title}})">评论管理
                  </el-button>
                </div>
            </div>
          </template>
  ...
  export default{
    ...
    methods: {
      ...
      GetKejianList(){
        ...
        this.$store.dispatch('GetKejianList', formData).then(res => {
          this.coursewareList = []
          res.kejianList.forEach((item)=> {
              ...
              this.coursewareList.push(item)
          })
          // this.coursewareList = res.coursewareList;
      ...
      GetKejianListByQuery(){
        ...
        this.$store.dispatch('GetKejianListByQuery', formData).then(res => {
          this.coursewareList = []
          res.kejiantList.forEach((item)=> {
              item.priceName = item.price+'元/年'
               switch(item.type){
                case 0:
                  item.typeName = '全部'
